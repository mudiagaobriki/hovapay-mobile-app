...
// Popular services data (reordered with insurance before bulk SMS)
...
export default function BillsScreen() {
    ...
    // Function to reorder service categories
    const getOrderedServiceCategories = () => {
        const apiCategories = serviceCategories?.content || [];
        const customCategories = [
            { identifier: 'insurance', name: 'Insurance' },
            { identifier: 'bulk-sms', name: 'Bulk SMS' }
        ];

        // Filter out other-services from API categories to add at the end
        const filteredApiCategories = apiCategories.filter(
            cat => cat.identifier !== 'other-services'
        );

        // Find other-services category from API
        const otherServices = apiCategories.find(
            cat => cat.identifier === 'other-services'
        );

        // Combine in desired order: API categories, insurance, bulk SMS, then other services last
